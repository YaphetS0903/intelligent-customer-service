...
三期业务工具底座内置“查询本人未读邮件数量”和“查询本人邮件摘要”两个只读工具。首次使用前，员工在 `/chat` 点击“连接邮箱”，使用自己的 Winmail 邮箱地址和密码完成一次真实身份验证。邮箱密码使用独立服务器密钥进行 AES-256-GCM 加密，密文与系统用户 ID 绑定；管理员页面和工具审计均不会返回密码、会话 `sessid` 或邮件正文。

```env
INTEGRATION_CREDENTIAL_ENCRYPTION_KEY=至少32位独立随机密钥
```

员工可以直接询问“我有多少封未读邮件”“查一下最近 5 封邮件”“查找今天来自某人的未读邮件”。系统只读取当前登录账号已验证邮箱的收件箱摘要，最多扫描前 5 页，保留主题、发件人、时间、未读状态、附件标记和大小；Winmail 返回的正文会在适配层立即丢弃。所有调用都经过工具启停、角色权限、参数 Schema、超时和只读重试控制，并记录调用人、参数摘要、耗时、结果计数与失败原因。管理员可在 `/admin/integrations` 的“业务工具”页停用工具并查看调用审计。

已有 MySQL 数据库升级时执行：

```bash
mysql -h "$MYSQL_HOST" -P "$MYSQL_PORT" -u "$MYSQL_USER" -p "$MYSQL_DATABASE" \
  < scripts/migrations/20260718_phase3_tool_gateway.mysql.sql
```

回滚脚本为 `scripts/migrations/20260718_phase3_tool_gateway.mysql.rollback.sql`。回滚会删除加密邮箱凭证、工具定义、调用审计和消息工具元数据，执行前必须先备份数据库。
